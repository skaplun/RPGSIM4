...
class WorldScene extends Phaser.Scene {
  ...
  createMap() {
    ...
    // creating the layers
	['Grass', 'Obstacles'].forEach(function(arg){
		this.map.createStaticLayer(arg, tiles, 0, 0);		
	}.bind(this));
  ...
  createPlayer(playerInfo) {
    ...
    this.container = this.add.container(playerInfo.worldData.position.x, playerInfo.worldData.position.y);
	...
	this.physics.add.overlap(this.weapon, this.otherPlayers, this.onMeetEnemy, false, this);
	// this.physics.add.collider(this.container, this.otherPlayers);
  }

  addOtherPlayers(playerInfo) {
    const otherPlayer = this.add.sprite(playerInfo.worldData.position.x, playerInfo.worldData.position.y, 'player', 9);
  ...
  switchToFight(){
	  this.scene.pause();
	  $('#world-game-container').addClass('hidden');
	  $('#fightScene').removeClass('hidden')
	  
  }
  
  onMeetEnemy(player, enemy) {
    if (this.attacking && this.otherPlayers.children.entries.length) {
	   this.switchToFight()
 	  f = new fightEngine(this.player.playerId, enemy.playerId, this.scene, this.socket);
	  f.initiate();
    }
  }
...
function socketConfig(){
	...
	this.socket.on('currentPlayers', function (players) {
      Object.keys(players).forEach(function (key) {
        if (players[key].id === this.socket.id) {
          ...
          this.addOtherPlayers(players[key]);
        }
      }.bind(this));
    }.bind(this));
    ...
    this.socket.on('playerMoved', function (playerInfo) {
      this.otherPlayers.getChildren().forEach(function (player) {
        if (playerInfo.id === player.playerId) {
          player.flipX = playerInfo.worldData.flipX;
          player.setPosition(playerInfo.worldData.position.x, playerInfo.worldData.position.y);
        }
      }.bind(this));
    }.bind(this));
    ...
    this.socket.on('new message', (data) => {
	 ...
	 this.socket.on('fightStart', function (fightData) {
		this.switchToFight();
		f = new fightEngine(fightData.enemy, fightData.player, this.scene, this.socket);
	 }.bind(this))
	 
	 this.socket.on('fightMap', function(data, player){
			f.getInitialFightData(data);
		})
		
	this.socket.on('fightPreUpdate', function(){ 
		f.updateServerOnFightDecisions();
	})
	
	this.socket.on('fightUpdate', function(data){ 
		f.updateFightBasedOnServer(data);
	})
	
	this.socket.on('getUpdatedFieldOfView', function(data){
		PubSub.publish('clearPreviouMove')
		f.updateFieldOfView(data);
	})
	
	this.socket.on('resumeWorld', function(data){ 
		f.UI.resumeWorld();
	})
}
